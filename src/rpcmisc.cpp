...
// Copyright (c) 2017-2018 The Linuxpay developers
...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total linuxpay balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"zLNPsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zLNP denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zLNP denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zLNP denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zLNP denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zLNP denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zLNP denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zLNP denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zLNP denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zLNP denominations\n"
            "  }\n"
            "  \"keypoololdest\": xxxxxx,    (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,        (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,      (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in linuxpay/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in linuxpay/kb\n"
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zlnpObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zlnpObj.emplace_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.emplace_back(Pair("zLNPsupply", zlnpObj));
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"linuxpayaddress\"\n"
            "\nReturn information about the given linuxpay address.\n"
            "\nArguments:\n"
            "1. \"linuxpayaddress\"     (string, required) The linuxpay address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"linuxpayaddress\", (string) The linuxpay address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Linuxpay address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are linuxpay addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) linuxpay address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"linuxpayaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"linuxpayaddress\"  (string, required) The linuxpay address to use for the signature.\n"
