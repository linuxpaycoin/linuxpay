                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zLNP ought to be enough for anybody." - Bill Gates, 2017
    ui->zLNPpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzLNPSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzLNP->setEnabled(false);
        ui->pushButtonMintzLNP->setToolTip(tr("zLNP is currently disabled due to maintenance."));

        ui->pushButtonSpendzLNP->setEnabled(false);
        ui->pushButtonSpendzLNP->setToolTip(tr("zLNP is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zLNPpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzLNP_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zLNP is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zLNP...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zLNP in ") +
...
void PrivacyDialog::on_pushButtonSpendzLNP_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zLNP is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zLNP
        sendzLNP();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zLNP
    sendzLNP();
}

void PrivacyDialog::on_pushButtonZLNPControl_clicked()
{
    ZLNPControlDialog* zLNPControl = new ZLNPControlDialog(this);
    zLNPControl->setModel(walletModel);
    zLNPControl->exec();
}

void PrivacyDialog::setZLNPControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzLNPSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzLNP()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Linuxpay Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zLNPpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zLNP is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " LNP </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zLNP</b>";
    ...
    // use mints from zLNP selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZLNPControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZLNPControlDialog::GetSelectedMints();
    }
    ...
    // Clear zlnp selector in case it was used
    ZLNPControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zLNP Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " LNP, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zLNP Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zLNP </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zLNP "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zLNP "));
    ui->labelzLNPAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzLNPSyncStatus->setVisible(fShow);
}
