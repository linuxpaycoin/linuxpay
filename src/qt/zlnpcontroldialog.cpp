...
#include "ui_zlnpcontroldialog.h"
...
std::list<std::string> ZLNPControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZLNPControlDialog::listMints;

ZLNPControlDialog::ZLNPControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZLNPControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZLNPControlDialog::~ZLNPControlDialog()
{
    delete ui;
}

void ZLNPControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZLNPControlDialog::updateList()
{
...
void ZLNPControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZLNPControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZLNP_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZLNPControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZLNPControlDialog::GetSelectedMints()
{
...
void ZLNPControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
